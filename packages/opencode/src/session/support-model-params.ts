 ...
 *
 * Model selection (3-tier fallback):
 * 1. Support agent's configured model (if exists)
 * 2. Calling agent's model (primary/subagent that invoked the tool)
...
export async function buildSupportModelParams(
  ...
  const modelInfo = useSupportAgent?.model
